...
| <setting v1="switch.{name}.input_pin_behavior" v2="switch.{name}.input_pin_behavior"></setting> | {::nomarkdown}<raw>momentary</raw>{:/nomarkdown} | If set to momentary when the input pin becomes high the switch changes to the ON state, and when it becomes low the switch changes to the OFF state. If set to toggle the input pin toggles the switch's state between ON and OFF. |
...
| <setting v1="switch.{name}.subcode" v2="switch.{name}.subcode"></setting> | {::nomarkdown}<raw>1</raw>{:/nomarkdown} | the subcode that the input on or input off commands respond to {::nomarkdown}<mcode>M106</mcode>{:/nomarkdown}.1 |
| <setting v1="switch.{name}.output_on_command" v2="switch.{name}.output_on_command"></setting> | {::nomarkdown}<raw>abort</raw>{:/nomarkdown} | This command is called when the switch changes to the ON state |
| <setting v1="switch.{name}.output_off_command" v2="switch.{name}.output_off_command"></setting> | {::nomarkdown}<raw>resume</raw>{:/nomarkdown} | This command is called when the switch changes to the OFF state |
...
| <setting v1="switch.{name}.output_type" v2="switch.{name}.output_type"></setting> | {::nomarkdown}<raw>pwm</raw>{:/nomarkdown} | Sets the type of output for the {::nomarkdown}<setting v1="switch.{name}.output_pin" v2="switch.{name}.output_pin"></setting>{:/nomarkdown}, if set to digital the pin can only be low or high, and if set to pwm (the default, using Sigma-Delta PWM) the pin can be set to any PWM value between 0 and 255 using the S parameter, for example : {::nomarkdown}<mcode>M106</mcode>{:/nomarkdown} S127. If set to hwpwm will use Real PWM, but the selected output pin must be [PWM capable](pwm-capable). The S value will be the duty cycle in percent. Can also be set to swpmw for software-emulated ( non-hardware ) pwm, that will be slower, but will not interfere with hardware pwm peripherals like a laser module. Can be set to none to disable the output entirely. |
| <setting v1="switch.{name}.startup_state" v2="switch.{name}.startup_state"></setting> | {::nomarkdown}<raw>false</raw>{:/nomarkdown} | Startup state of the switch. If set to false the module is initialized OFF, if set to true the module is initialized ON |
| <setting v1="switch.{name}.startup_value" v2="switch.{name}.startup_value"></setting> | {::nomarkdown}<raw>50</raw>{:/nomarkdown} | Startup value of the switch if the <setting v1="switch.{name}.output_type" v2="switch.{name}.output_type"></setting> is any kind of pwm. Value range is 0-100 (percentage). <setting v1="switch.{name}.startup_state" v2="switch.{name}.startup_state"></setting> must be false for this to take effect. It is also the value the hwpwm or swpwm is set to on HALT. |
| <setting v1="switch.{name}.default_on_value" v2="switch.{name}.default_on_value"></setting> | {::nomarkdown}<raw>50</raw>{:/nomarkdown} | Default on setting value of the switch if the <setting v1="switch.{name}.output_type" v2="switch.{name}.output_type"></setting> is swpwm or hwpwm. Value range is 0-100 (percentage). It is also the value the hwpwm or swpwm is set to when <setting v1="switch.{name}.startup_state" v2="switch.{name}.startup_state"></setting> is true |
| <setting v1="switch.{name}.max_pwm" v2="switch.{name}.max_pwm"></setting> | {::nomarkdown}<raw>210</raw>{:/nomarkdown} | Maximum value for the PWM output. (only used for pwm output type, not for hwpwm) |
| <setting v1="switch.{name}.pwm_period_ms" v2="switch.{name}.pwm_period_ms"></setting> | {::nomarkdown}<raw>20</raw>{:/nomarkdown} | Period used by the H/W and S/W PWM, 20ms is 50Hz which is the default if not set |
| <setting v1="switch.{name}.halt_set_to" v2="switch.{name}.halt_set_to"></setting> | {::nomarkdown}<raw>false</raw>{:/nomarkdown} | true or false what state to set the switch to when a HALT condition is triggered (M112). For digital outputs sets pin high (true) or low (false). For PWM outputs (hwpwm/swpwm) the startup_value is used. Different from failsafe_set_to which handles crash/debug conditions. |
| <setting v1="switch.{name}.failsafe_set_to" v2="switch.{name}.failsafe_set_to"></setting> | {::nomarkdown}<raw>0</raw>{:/nomarkdown} | 0 or 1 what to set the output pin to in case of a crash, watchdog reset, or debug condition. Does NOT apply to M112 HALT (see halt_set_to for that). |
| <setting v1="switch.{name}.ignore_on_halt" v2="switch.{name}.ignore_on_halt"></setting> | {::nomarkdown}<raw>false</raw>{:/nomarkdown} | set to true to not set the failsafe or <setting v1="switch.{name}.startup_value" v2="switch.{name}.startup_value"></setting> value when a HALT condition is triggered. Note: automatically set to true for input-pin switches and cannot be overridden. |
