...
<script>
// Display and manage review data
(function() {
    ...
    function displayReviewData() {
        ...
        const jsonElement = document.getElementById('review-json');

        if (Object.keys(reviews).length === 0) {
            jsonElement.textContent = '(No review data yet - interact with reviews above to see data here)';
        } else {
            jsonElement.textContent = JSON.stringify(reviews, null, 2);
        }
    }
    ...
    function showFeedback(message, variant = 'success') {
        ...
        feedback.innerHTML = `
            <sl-alert variant="${variant}" open closable>
                <sl-icon slot="icon" name="${variant === 'success' ? 'check-circle' : 'info-circle'}"></sl-icon>
                ${message}
            </sl-alert>
        `;
    ...
    document.getElementById('refresh-data-button').addEventListener('click', () => {
        displayReviewData();
        showFeedback('Review data refreshed');
    });
    ...
    document.getElementById('export-data-button').addEventListener('click', async () => {
        ...
        try {
            ...
            showFeedback('Review JSON copied to clipboard!');
        } catch (err) {
            showFeedback('Failed to copy to clipboard. Check console for data.', 'warning');
            console.log('Review data:', reviews);
        }
    });

    // Listen for storage changes (from review tag interactions)
    ...
    // Also refresh every 2 seconds to catch same-page changes
    setInterval(displayReviewData, 2000);
})();
</script>
