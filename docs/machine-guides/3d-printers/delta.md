...
Test the endstops with {::nomarkdown}<mcode>M119</mcode>{:/nomarkdown}, and make sure the X endstop triggers when the endstop on the X tower is activated, and same for Y and Z towers.
...
{::nomarkdown}<mcode>M666</mcode>{:/nomarkdown} Xnnn Ynnn Znnn allow you to adjust the trim values on a live system.

You need to home after setting {::nomarkdown}<mcode>M666</mcode>{:/nomarkdown}.

Negative values adjust the endstops down by that number of mm. Positive values are up.

When you have found the correct values for your endstop settings (see the suggested workflow near the bottom of this page for one process to adjust the trim values), edit the config file and reboot, or save the {::nomarkdown}<mcode>M666</mcode>{:/nomarkdown} values with {::nomarkdown}<mcode>M500</mcode>{:/nomarkdown}.
...
These can also be saved with {::nomarkdown}<mcode>M500</mcode>{:/nomarkdown}.
...
2. Home, with {::nomarkdown}<gcode>G28</gcode>{:/nomarkdown}.
...
5. Do {::nomarkdown}<mcode>M306</mcode>{:/nomarkdown} Z0.

6. Home again, with {::nomarkdown}<gcode>G28</gcode>{:/nomarkdown}.
...
9. Tune the Y (beta) endstop offset value by entering {::nomarkdown}<mcode>M666</mcode>{:/nomarkdown} Ynnn where `nnn` is the distance between the hotend and bed at this point.

10. Home the machine with {::nomarkdown}<gcode>G28</gcode>{:/nomarkdown}
...
13. Tune the Z endstop offset value by entering {::nomarkdown}<mcode>M666</mcode>{:/nomarkdown} Znnn where `nnn` is the distance between the hotend and bed at this point.

14. Home the machine with {::nomarkdown}<gcode>G28</gcode>{:/nomarkdown}.
...
16. Now that all three towers' endstop offsets are tuned, save them with {::nomarkdown}<mcode>M500</mcode>{:/nomarkdown}.

17. Test the height is now the same at the three points as it is at the center, if it is not, Adjust your arm radius with {::nomarkdown}<mcode>M665</mcode>{:/nomarkdown} Rnnn.

18. Continue adjusting arm radius until the three points and the center of the bed are all at the same height.

19. Finally, go to the center of the bed, move the head until it touches the bed, and issue {::nomarkdown}<mcode>M306</mcode>{:/nomarkdown} Z0.

20. Save all your new settings with {::nomarkdown}<mcode>M500</mcode>{:/nomarkdown}.
...
Alternatively, the values can be changed live (and saved with {::nomarkdown}<mcode>M500</mcode>{:/nomarkdown}):
