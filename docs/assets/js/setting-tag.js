...
`);
  ...
  import_jquery.default(() => {
    ...
    const $setting_elements = import_jquery.default("setting");
  ...
  function create_popup_for_setting($setting, v1_name, v2_name) {
    ...
    const $popup = import_jquery.default(`
        <sl-popup
            id="${popup_id}"
            placement="bottom"
            distance="10"
  ...
  function setup_related_settings_handlers($popup, popup_id) {
    setTimeout(() => {
      $popup.find(".related-setting-button").on("click", async function() {
        const setting_name = import_jquery.default(this).data("setting-name");
        console.log(`[setting-tag.ts] Clicked related setting: ${setting_name}`);
        const active_tab_element = $popup.find("sl-tab[active]")[0];
        const active_panel_name = active_tab_element ? import_jquery.default(active_tab_element).attr("panel") : "v1-panel";
    ...
    setTimeout(() => {
      ...
      if (tab_group && clickable_alerts.length > 0) {
        clickable_alerts.each((_index, alert) => {
          const $alert = import_jquery.default(alert);
          const target_panel = $alert.attr("data-target-panel");
          if (target_panel) {
            $alert.on("click", (e) => {
              if (import_jquery.default(e.target).closest("sl-copy-button").length > 0) {
                return;
              }
    ...
    setTimeout(() => {
      ...
      if (clickable_lines.length > 0) {
        clickable_lines.each((_index, line) => {
          const $line = import_jquery.default(line);
          ...
          if (github_url) {
            ...
            $line.on("mouseenter", function() {
              import_jquery.default(this).css("background-color", "rgba(88, 166, 255, 0.15)");
            });
