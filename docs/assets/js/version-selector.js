...
function update_version_selector_button() {
  ...
  const $button = import_jquery.default("#version-selector-button");
  $button.attr("data-version", display_version);
  const $menu_items = import_jquery.default(".version-menu-item");
  $menu_items.attr("data-current", "false");
  $menu_items.each(function() {
    const $item = import_jquery.default(this);
...
function setup_version_selector_handlers() {
  import_jquery.default(".version-menu-item").on("click", function() {
    const $item = import_jquery.default(this);
    const selected_version = $item.attr("data-version");
    set_display_version(selected_version);
    update_version_selector_button();
    const $dropdown = import_jquery.default("#version-selector-dropdown")[0];
    if ($dropdown && $dropdown.hide) {
      $dropdown.hide();
    }
    import_jquery.default(document).trigger("version-changed", [selected_version]);
  });
}
import_jquery.default(document).ready(function() {
  update_version_selector_button();
