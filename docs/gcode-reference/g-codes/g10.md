...
{::nomarkdown}<gcode>G10</gcode>{:/nomarkdown} means "retract the filament from the hotend using the extruder."

You need to configure the length to retract in your [extruder module](extruder)'s [configuration](configuring-smoothie) by setting the {::nomarkdown}<setting v1="extruder.hotend.retract_length" v2="extruder.hotend.retract_length"></setting>{:/nomarkdown} parameter.

By default, most slicing programs will use {::nomarkdown}<gcode>G1</gcode>{:/nomarkdown} commands to retract, but this is the more modern way of doing it.

{::nomarkdown}<gcode>G10</gcode>{:/nomarkdown}'s evil twin is {::nomarkdown}<gcode>G11</gcode>{:/nomarkdown}, which means "unretract the filament from the hotend using the extruder."
