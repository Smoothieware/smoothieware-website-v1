...
<!-- learning-diagram:20-m23-select-file -->
{::nomarkdown}
<figure id="learning-diagram-20-m23-select-file" style="clear: both; max-width: 960px; margin: 2rem auto; scroll-margin-top: 16vh;">
  <a href="/images/learning-diagrams/20-m23-select-file.svg">
    <img src="/images/learning-diagrams/20-m23-select-file.svg" alt="M23 selects a file from the SD card: The file remains selected until another M23 command replaces it." loading="lazy" decoding="async" style="display: block; width: 100%; height: auto; border-radius: 0.1rem;"/>
  </a>
  <figcaption style="margin-top: 0.6rem; font-size: 0.92rem; opacity: 0.82; text-align: center;">M23 selects a file from the SD card <a href="#learning-diagram-20-m23-select-file" aria-label="Permanent link to M23 selects a file from the SD card" style="margin-left: 0.35rem; text-decoration: none;">#</a></figcaption>
</figure>
{:/nomarkdown}
...
When using {::nomarkdown}<mcode>M23</mcode>{:/nomarkdown} to select and {::nomarkdown}<mcode>M24</mcode>{:/nomarkdown} to start files, the Player module will automatically execute macro files if they exist:
