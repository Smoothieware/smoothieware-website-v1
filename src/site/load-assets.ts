...
// Extend window interface to include our global settings data and config files
declare global {
    interface Window {
        v1_settings?: SettingsData;
        v2_settings?: SettingsData;
        v1_config_file?: string;
        v2_config_file?: string;
        settings_data_loaded?: boolean;
        config_files_loaded?: boolean;
    }
}

// Type definitions for settings data structure
...
interface Setting {
    ...
    context?: string;
    defined_in?: string;
    corresponding_v1?: string;
    corresponding_v2?: string;
    description: string;
    description_bullets?: string[];
    typical_values?: SettingValue[];
...
interface SettingsData {
    ...
    settings_by_name: Map<string, Setting>;
    settings_by_corresponding: Map<string, Setting>;  // Lookup by corresponding_v1 or corresponding_v2 values
}
 ...
 */
async function load_settings_data(): Promise<void> {
    ...
    try {
        ...
        // Parse YAML
        const v1_data = yaml.load(v1_yaml_text) as any;
        const v2_data = yaml.load(v2_yaml_text) as any;
 ...
 */
function check_cache_validity(): boolean {
 ...
 */
function process_settings_data(raw_data: any): SettingsData {

    // Create Map for fast setting lookup by name
    const settings_by_name = new Map<string, Setting>();

    // Create Map for lookup by corresponding_v1/v2 values
    const settings_by_corresponding = new Map<string, Setting>();
 ...
 */
function reconstruct_settings_data(cached_data: any): SettingsData {
 ...
 */
function cache_settings_data(v1_data: any, v2_data: any): void {
 ...
 */
async function load_config_files(): Promise<void> {
 ...
 * Returns true if cache exists and is not expired
 */
function check_config_cache_validity(): boolean {
 ...
 */
function cache_config_files(v1_config: string, v2_config: string): void {
