 ...
 */
function load_review_state(review_id: string): ReviewState {
    try {
        ...
        const reviews: ReviewsData = JSON.parse(reviews_json);
 ...
 */
function save_review_state(review_id: string, state: ReviewState): void {
    try {
        // Get existing reviews
        const reviews_json = localStorage.getItem(REVIEWS_STORAGE_KEY);
        const reviews: ReviewsData = reviews_json ? JSON.parse(reviews_json) : {};
 ...
 */
function show_storage_error_dialog(error_message: string): void {
 ...
 */
function validate_review_id(review_id: string): boolean {
    // Expected format: "page-name:description"
    const valid_format = /^[a-z0-9-]+:[a-z0-9-]+$/i;

    if (!valid_format.test(review_id)) {
        console.warn('[review-tag.ts] Invalid review ID format:', review_id,
                     'Expected format: "page-name:description"');
        return false;
    }
 ...
 */
function detect_duplicate_review_ids(): void {
    const review_ids: { [id: string]: number } = {};
 ...
 */
function update_review_visual_state($review_element: JQuery<HTMLElement>, state: ReviewState): void {
 ...
 */
function toggle_view($review_element: JQuery<HTMLElement>): void {
    const $proposal = $review_element.find('.review-proposal');
    const $original = $review_element.find('.review-original');
 ...
 */
function handle_accept(review_id: string, $review_element: JQuery<HTMLElement>): void {
    // Load current state
    ...
    if (state.accept) {
        ...
        const new_state: ReviewState = { accept: true };
 ...
 */
function handle_reject(review_id: string, $review_element: JQuery<HTMLElement>): void {
    // Load current state
    const state = load_review_state(review_id);
    ...
    if (state.reject) {
        ...
        const new_state: ReviewState = { reject: true };
        save_review_state(review_id, new_state);
        update_review_visual_state($review_element, new_state);
    }
}
 ...
 */
function save_comment(review_id: string, note_text: string, $review_element: JQuery<HTMLElement>): void {
 ...
 */
function handle_comment(review_id: string, $review_element: JQuery<HTMLElement>): void {
    // Load current state
    const state = load_review_state(review_id);
    ...
    setTimeout(() => {
        ...
        const dialog_element = $dialog.get(0) as any;
        ...
        $dialog.find('.review-dialog-save').on('click', function() {
            ...
            const textarea_element = $textarea.get(0) as any;
 ...
 */
function reprocess_custom_tags($review_element: JQuery<HTMLElement>): void {
    ...
    if (typeof (window as any).reprocess_setting_tags === 'function') {
        (window as any).reprocess_setting_tags($setting_tags);
    }

    if (typeof (window as any).reprocess_pin_tags === 'function') {
        (window as any).reprocess_pin_tags($pin_tags);
    }

    if (typeof (window as any).reprocess_versioned_tags === 'function') {
        (window as any).reprocess_versioned_tags($versioned_tags);
    }
}
 ...
 */
function build_review_structure($review_element: JQuery<HTMLElement>): void {
 ...
 */
function initialize_review_tags(): void {
