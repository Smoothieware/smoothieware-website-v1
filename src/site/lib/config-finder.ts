...
// Interface for config file metadata
export interface ConfigFileMetadata {
    path: string;                    // Path relative to assets/config/
    github_repo: string;             // GitHub repository name (e.g., "Smoothieware" or "SmoothieV2")
    github_path: string;             // Path in GitHub repo (e.g., "ConfigSamples/config-3d.ini")
    priority: number;                // Search priority (lower = search first)
}

// Interface for the search request
export interface ConfigSearchRequest {
    setting_name: string;           // The setting to search for
    version: 'v1' | 'v2';          // Config version
    config_content?: string;        // Optional: single config file content (legacy)
    module_name?: string;           // For v1: module name (first part of path)
    section_name?: string;          // For v2: section name from YAML
}

// Interface for a display line with metadata
export interface ConfigLine {
    line_number: number;            // Original line number in config file
    content: string;                // The line content (plain text)
    html_content?: string;          // Optional: syntax-highlighted HTML version
    is_target: boolean;             // Is this the target setting line?
    is_section_header: boolean;     // Is this a section header (v2 only)
    fade_level: number;             // 0 = full opacity, 1-3 = fade levels, higher = more faded
}

// Interface for the search result
export interface ConfigSearchResult {
    found: boolean;                 // Whether the setting was found
    lines: ConfigLine[];           // Lines to display (with fade context)
    target_line_index: number;      // Index in lines array of the target setting (-1 if not found)
    section_header_index: number;   // Index in lines array of section header (-1 if none/not found)
    file_path: string;              // Relative path of file where setting was found
    github_url: string;             // Full GitHub URL to the file
    error_message?: string;         // Error message if not found
}

// Define all config files to search (in priority order)
const V1_CONFIG_FILES: ConfigFileMetadata[] = [
...
const V2_CONFIG_FILES: ConfigFileMetadata[] = [
 ...
 */
function generate_github_url(metadata: ConfigFileMetadata, line_number: number): string {
 ...
 */
function apply_syntax_highlighting(line: string): string {
 ...
 */
export async function find_setting_in_config(request: ConfigSearchRequest): Promise<ConfigSearchResult> {
...
function find_setting_v1(
    ...
    const lines_with_context = add_fade_context(
        all_lines,
        module_lines,
        setting_line_info.line_number,
        -1  // No section header in v1
    );
 ...
 */
function normalize_section_name(name: string): string {
...
function find_setting_v2(
    ...
    let clean_section: string | null = null;
    if (section_name) {
    ...
    let expected_section: string | null = clean_section;
    ...
    const lines_with_context = add_fade_context(
        ...
        section_lines,
        setting_line_info.line_number,
        section_info.start_line  // Section header line
    );
 ...
 */
function find_setting_line_v1(
    setting_name: string,
    all_lines: string[]
): { line_number: number; is_commented: boolean } {
 ...
 */
function extract_module_from_setting_name(setting_name: string): string {
 ...
 */
function find_module_section_v1(
    all_lines: string[],
    module_name: string,
    anchor_line: number
): number[] {

    const module_lines: number[] = [];
 ...
 */
function is_section_boundary_v1(line: string): boolean {
 ...
 */
function is_comment_line_v1(line: string): boolean {
 ...
 */
function find_section_for_line_v2(
    all_lines: string[],
    target_line: number
): { start_line: number; end_line: number; section_name: string } {
 ...
 */
function add_fade_context(
    all_lines: string[],
    section_lines: number[],
    target_line: number,
    section_header_line: number
): ConfigLine[] {
